...
The package only has one function which allows to extract all of the
data available in the Cook County State Attorney’s Open Data Portal.
...
There are 4 datasets included in the package. All of them are filtered
for cases **received** by the State Attorney’s Office during 2019. If
you wish to extract cases from before or after 2019 you’ll need to use
the `get_all_data()` function. This is given size constraints for
packages.
...
Datasets:
  ...
  - **Dispositions**
      ...
      - *Rows*: 20,234
      - *Variables*: 31
      - *Description*: How the charges turned out (guilty, not guilty,
        dismissed, etc)
  - **Sentencing**
      - *Observational Unit*: Charge
      - *Rows*: 4,329
